...
**Approach:**
- Tables: `Job` (company, position, description, url, source enum manual|recommended, status enum not_started|applied|denied|accepted, `dismissed_at` nullable timestamp, match_score/rationale nullable, `source_ref` with partial unique index where not null, `dedup_hash`, timestamps), `StatusEvent` (job_id, from/to status, at), `Comment`, `Contact`, `JobLink` (all job_id FK), `ResumeSection` (name, content, position), `TailoredResume` (job_id FK, `is_submitted` bool, created_at) + `TailoredSection` (tailored_resume_id, `base_section_id` nullable FK `ON DELETE SET NULL`, copied section name + order, content, changed flag), `ConfigProfile` (experience, location, pay_min), `ExperienceSource` (type enum resume|case_study|webpage|manual, content/url).
- Invariants stated here, tested here and downstream: tailored sections are fully self-contained (render with zero live master references); `Job.status` mutates only through the transition path; dedup is DB-enforced.
...
- Happy path: status transition writes a `StatusEvent` row and updates `Job.status` in one transaction
...
**Approach:**
- `GET /jobs` returns list rows that embed everything Nathan's board row needs without follow-up calls: company, position, status, source, match_score/rationale, derived `applied_at` (latest `applied` StatusEvent), derived `last_activity_at` (latest of status event / comment / job update), and counts of comments/contacts/links. Query params: `status`, `include_dismissed` (default false → `dismissed_at IS NULL`), free-text `q` (company/position/description `LIKE`), sort by `last_activity_at`.
- `POST /jobs` (manual add — computes `dedup_hash`), `GET/PATCH/DELETE /jobs/{id}`; PATCH recomputes `dedup_hash` when company/position change and **rejects a `status` field** (422) — status moves only via `POST /jobs/{id}/status`, which writes the StatusEvent and updates the column transactionally. All pipeline transitions are allowed (including denied → applied for re-applications); history lives in StatusEvent.
...
**Test scenarios:**
- Happy path: add job → appears in list; transition to `applied` → StatusEvent recorded and list row's `applied_at` populated; add comment/contact/link → counts reflected in the list row
...
- Happy path: denied → applied is allowed, produces a third StatusEvent, and `applied_at` now reflects the latest `applied` transition
- Edge case: empty list returns 200 `[]`; unknown job id returns 404; invalid status value returns 422
- Error path: `PATCH /jobs/{id}` containing `status` → 422 (single-writer invariant)
- Error path: DELETE on a recommended job → 409; DELETE on a job with a *submitted* tailored resume → 409 with actionable message
- Integration: full lifecycle — create → applied → denied → applied — leaves the correct event trail with correct dates

**Verification:** Every landing-page field on Nathan's board (company, position, status, dates, comments, contacts, links, search/filter) is served by one documented list endpoint; the status/date audit trail is immune to generic PATCH.
...
- **Status-tracking design: `StatusEvent` audit table vs simple per-transition timestamp columns.** Scope-guardian argues the event table + single-writer invariant + re-application flow exceeds the origin's "no sub-stages until real use demands them" philosophy, and `applied_at`/`denied_at`/`accepted_at` columns would satisfy R1's "dates" more simply. Adversarial's counterpoint: whichever design wins, **backdating must be supported** — backfilled applications (entered after the fact) need a real applied date, so either the transition endpoint takes an optional `at` timestamp (StatusEvent design) or the date columns are directly editable (columns design). The plan currently specifies the StatusEvent design; Josh should rule on this before U2 lands the schema. (scope-guardian + adversarial, both confidence 75)
